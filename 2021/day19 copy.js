...
function part1 (input) {
  ...
  for (const id in scanners) {
    const scanner = scanners[id]
    const distances = {}
    for (let i = 0; i < scanner.length - 1; i++) {
      const [ix, iy, iz] = scanner[i]
      for (let j = i + 1; j < scanner.length; j++) {
        const [jx, jy, jz] = scanner[j]
        let dist = [ix - jx, iy - jy, iz - jz]
        dist = dist.map(d => Math.abs(d))
        dist.sort((a, b) => a - b)
        distances[dist.join(',')] = [i, j]
      }
    }
