...
function part1 (input) {
  ...
  function parse (bin) {
    let version = parseInt(bin.splice(0, 3).join(''), 2)
    let typeId = parseInt(bin.splice(0, 3).join(''), 2)
    if (typeId === 4) {
    ...
    let results = []
    let i = bin.splice(0, 1)[0]
    if (i === '0') {
      let l = parseInt(bin.splice(0, 15).join(''), 2)
      let tl = bin.length - l
      while (bin.length > tl) {
        let res = parse(bin)
        results.push(res)
      ...
      let l = parseInt(bin.splice(0, 11).join(''), 2)
