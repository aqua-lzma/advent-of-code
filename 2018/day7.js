...
function f (data) {
  ...
  while (!mapped) {
    ...
    for (const [parent, child] of data) {
      if (parent in map) {
        const self = child in map ? map[child] : { name: child, children: [], parents: [] }
        map[child] = self
